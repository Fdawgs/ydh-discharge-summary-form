		...
		await request
			...
			.then((body) => {
				...
				if (result.identifier) {
					for (
						let index = 0;
						index < result.identifier.length;
						index += 1
					) {
						const element = result.identifier[index];
						if (
							element.use.toString().toLowerCase() ===
								'official' &&
						...
						if (
							element.use.toString().toLowerCase() === 'usual' &&
							element.system.toString() ===
				...
				if (result.contained) {
					for (
						let index = 0;
						index < result.contained.length;
						index += 1
					) {
						const element = result.contained[index];
						if (
							element.resourceType.toString().toLowerCase() ===
								'organization' &&
							element.id.toString() ===
							...
							params.gp_address = `${element.address[0].line.toString()}, ${
