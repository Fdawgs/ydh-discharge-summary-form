...
return _classCallCheck(this,a),this.config=b,this.app=express(),this.app.set("view engine","ejs"),this.app.use(express["static"](path.join(__dirname,"../assets"))),this.app.set("views",path.join(__dirname,"../views")),this}/**
	 * @author Frazer Smith
...
return this.app.get("/searchpatient",fhirPatientRecord(a),function(a,b){b.render("./pages/patient_confirmation",a.patientresource)}),this.app.post("/createnew",gatherCustomParams(),fhirAllergyRecord(a),insertUpdateRecord(this.pool),retrieveInProgress(this.pool),function(a,b){return regeneratorRuntime.async(function(c){for(;;)switch(c.prev=c.next){case 0:b.render("./pages/patient_landing",a.customparams);case 1:case"end":return c.stop();}})}),this}/**
	 * @author Frazer Smith
